...
import { Expand, Eye, Gift, Heart, MessageSquare, UserPlus } from 'lucide-react';
import './index.scss';
...
const StreamPageTempalate = () => {
  ...
  useEffect(() => {
    if (streamerID) {
      axios.request(getAxiosParam(`${process.env.NEXT_PUBLIC_API_URL}/follows/counts`, 'POST', { user_id: streamerID }, '', { withCredentials: true })).then((res) => {
  ...
  useEffect(() => {
    if (streamerID) {
      const paramStream = getAxiosParam(`${process.env.NEXT_PUBLIC_API_URL}/streams/${streamerID}`, 'GET', {}, '', { withCredentials: true, });
  ...
  const donate = () => {
    const { config } = usePrepareContractWrite({
      address: '0x5de39cAEeB1FAb1e1242A58e8a7B5335249563e3',
      abi: donateABI.abi,
      functionName: 'egg',
      args: [
        '0xFAD8e802b189d935A0C0F5fE18c2Ec22DA9B8587'
      ],
      value: BigInt(ethers.formatEther(1000000000)),
    });
  }
      ...
      <div className='basis-3/5 h-full'>
        {
          ...
          <section className='flex h-full gap-2 flex-col'>
            <div className='w-full h-full'>
              {streamData?.video_urls ? (<View className='h-full' url={streamData?.video_urls} videoID={streamData?.video_id} />) : <VideoSkeleton className='h-full' />}
            </div>
            {
              streamData ?
                (
                  ...
                  <Skeleton className='border p-4 rounded-lg max-w-full' >
                    <Skeleton className="mx-auto">
                      <Skeleton className="card md:flex w-[90%]">
                        <Skeleton className="w-20 h-20mx-auto mb-6 md:mr-6 object-cover rounded-full  bg-gray-300 ">
                        </Skeleton>
                        <Skeleton className="flex flex-col  w-full gap-2 flex-grow text-center md:text-left">
                          <Skeleton className="text-gray-300 bg-gray-300 w-full">Senior Developer</Skeleton>
                          <Skeleton className="text-xl heading text-gray-300 bg-gray-300  w-full">John Doe</Skeleton>
                          <Skeleton className="mt-2 mb-3 text-gray-300 bg-gray-300  w-full">John is a Senior Developer, mainly works in backend technologies.</Skeleton>
                          <Skeleton className='flex gap-2'>
                            <Skeleton className="border px-3 py-1.5 rounded-lg text-sm text-gray-300 bg-gray-300">Discrete Math</Skeleton>
                            <Skeleton className="border px-3 py-1.5 rounded-lg text-sm text-gray-300 bg-gray-300">Topology</Skeleton>
                            <Skeleton className="border px-3 py-1.5 rounded-lg text-sm text-gray-300 bg-gray-300">Neural Nets</Skeleton>
                          </Skeleton>
                        </Skeleton>
                      </Skeleton>
                    </Skeleton>
